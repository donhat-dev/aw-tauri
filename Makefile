...
bundle-watchers:
ifeq ($(OS),Windows_NT)
	@powershell -NoProfile -ExecutionPolicy Bypass -Command '$$watchers = "$(TAURI_WATCHERS)".Split(" ", [System.StringSplitOptions]::RemoveEmptyEntries); New-Item -ItemType Directory -Force "resources" | Out-Null; foreach ($$watcher in $$watchers) { $$srcDir = Join-Path "$(WATCHER_DIST)" (Join-Path $$watcher (Join-Path "dist" $$watcher)); $$srcExe = Join-Path "$(WATCHER_DIST)" (Join-Path $$watcher ("dist/" + $$watcher + ".exe")); $$target = Join-Path "resources" $$watcher; if (Test-Path -LiteralPath $$target) { Remove-Item -LiteralPath $$target -Recurse -Force }; if (Test-Path -LiteralPath $$srcDir -PathType Container) { New-Item -ItemType Directory -Force $$target | Out-Null; Copy-Item -Path (Join-Path $$srcDir "*") -Destination $$target -Recurse -Force; Write-Host "Bundled $$watcher from $$srcDir" } elseif (Test-Path -LiteralPath $$srcExe -PathType Leaf) { New-Item -ItemType Directory -Force $$target | Out-Null; Copy-Item -LiteralPath $$srcExe -Destination (Join-Path $$target ($$watcher + ".exe")) -Force; Write-Host "Bundled $$watcher from $$srcExe" } else { Write-Host "Skipping $$watcher (not found at $$srcDir or $$srcExe)" } }'
else
	@mkdir -p resources
